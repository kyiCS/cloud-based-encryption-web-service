...
const sendEmails = async (event) => {
  ...
  const emailBodyText =
    `Encryption done for "${fileName}"\n` +
    `Download the encrypted version of your file using this link: ${dowloadUrl}"\n` +
  ...
  const emailBodyHtml = `<html>
        <head></head>
        <body>
        <h1>Encryption done for "${fileName}"</h1>
        <h2>Download the encrypted version of your file using <a href="${dowloadUrl}">this link</a></h2>
  ...
  const sendEmailSettings = {
    ...
    Message: {
      ...
      Subject: { Charset: "UTF-8", Data: "Encryption Done" },
    },
